...
VAR_CHECK_HII_VARIABLE_HEADER  *mMmRecievedVarCheckHiiBin    = NULL;
UINTN                          mMmRecievedVarCheckHiiBinSize = 0;
  ...
  )
{
  ...
  mMmRecievedVarCheckHiiBinSize = *CommBufferSize;

  if (mMmRecievedVarCheckHiiBinSize < sizeof (VAR_CHECK_HII_VARIABLE_HEADER)) {
    DEBUG ((DEBUG_ERROR, "%a: MM Communication buffer size is invalid for this handler!\n", __func__));
    return EFI_ACCESS_DENIED;
  }

  mMmRecievedVarCheckHiiBin = AllocateZeroPool (mMmRecievedVarCheckHiiBinSize);
  if (mMmRecievedVarCheckHiiBin == NULL) {
    DEBUG ((DEBUG_ERROR, "%a: Failed to allocate memory for mVarCheckHiiBinMm\n", __func__));
    return EFI_OUT_OF_RESOURCES;
  }

  CopyMem (mMmRecievedVarCheckHiiBin, CommBuffer, mMmRecievedVarCheckHiiBinSize);
  return EFI_SUCCESS;
}
...
{
  return CheckHiiVariableCommon (mMmRecievedVarCheckHiiBin, mMmRecievedVarCheckHiiBinSize, VariableName, VendorGuid, Attributes, DataSize, Data);
}
  ...
  )
{
  ...
  VarCheckLibRegisterAddressPointer ((VOID **)&mMmRecievedVarCheckHiiBin);
